...
**Vulnerability:** Command injection vulnerability due to string interpolation when executing PowerShell commands with untrusted inputs (`vmName` and `vhdxPath`).
**Learning:** Using simple string replacement like `.Replace("'", "''")` is inadequate and prone to edge cases for PowerShell sanitization.
**Prevention:** Always use `ProcessStartInfo.EnvironmentVariables` to pass untrusted inputs instead of character escaping.
