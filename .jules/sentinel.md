...
**Vulnerability:** Command injection was possible in PowerShell executions because untrusted variables (like `vmName`, `vhdxPath`, and WMI outputs) were concatenated directly into the command string.
**Learning:** Using `$@"{variable}"` and character escaping logic (like replacing `'` with `''`) is brittle and insecure for passing data to external shells.
**Prevention:** Pass variables to PowerShell scripts using environment variables (`ProcessStartInfo.EnvironmentVariables`) instead of string interpolation.
