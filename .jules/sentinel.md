...
## 2024-05-24 - Secure PowerShell Execution
**Vulnerability:** Command injection via string interpolation in PowerShell commands.
**Learning:** Even with basic sanitization (e.g., replacing `'` with `''`), interpolating variables directly into PowerShell command strings leaves the application vulnerable to injection.
**Prevention:** Always pass untrusted data securely via the ProcessStartInfo.Environment dictionary and reference it as `$env:VARNAME` within the PowerShell command string.
