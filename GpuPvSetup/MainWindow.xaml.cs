...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            Dictionary<string, string>? envVars = new Dictionary<string, string> { { "VM_NAME", vmName } };
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    string vhdxPathLocal = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(vhdxPathLocal))
                    {
                         envVars["VHDX_PATH"] = vhdxPathLocal;
