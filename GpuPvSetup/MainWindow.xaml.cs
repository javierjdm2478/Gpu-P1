    ...
    public partial class MainWindow : Window
    {
        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            try
            {
                ...
                var envVarsVhdx = new Dictionary<string, string> { { "VHDX_PATH", vhdxPath } };
                string scriptMount = @"
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    {
                         var envVarsVhdx = new Dictionary<string, string> { { "VHDX_PATH", vhdxPath } };
                         RunPowerShellCommand("Dismount-VHD -Path $env:VHDX_PATH", envVarsVhdx);
        ...
        private (string Vendor, string DriverPath) DetectHostGpuAndDriver()
        {
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_VideoController"))
            {
                foreach (ManagementObject obj in searcher.Get())
                {
                    ...
                    var envVarsGpu = new Dictionary<string, string> { { "GPU_NAME", name } };
                    string script = @"
                    ...
                    string driverPath = RunPowerShellCommand(script, envVarsGpu).Trim();
