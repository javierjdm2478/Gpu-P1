    ...
    public partial class MainWindow : Window
    {
        ...
        private async Task LoadVirtualMachines()
        {
            ...
            try
            {
                ...
                var vms = await Task.Run(() => RunPowerShellCommand("Get-VM | Select-Object -ExpandProperty Name", null));
        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    string cleanupVhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(cleanupVhdxPath))
                    {
                         envVars["CLEANUP_VHDX_PATH"] = cleanupVhdxPath;
                         RunPowerShellCommand("Dismount-VHD -Path $env:CLEANUP_VHDX_PATH", envVars);
