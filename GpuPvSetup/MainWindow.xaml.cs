...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            var envVm = new Dictionary<string, string> { { "VM_NAME", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:VM_NAME | Select-Object -ExpandProperty State", envVm).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:VM_NAME -Force", envVm);
                ...
                RunPowerShellCommand("Set-VM -Name $env:VM_NAME -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", envVm);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:VM_NAME", envVm);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", envVm).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim() ?? string.Empty;
                ...
                string scriptMount = @"
                    ...
                    $vol.DriveLetter
                ";
                var envVhdx = new Dictionary<string, string> { { "VHDX_PATH", vhdxPath } };
                string driveLetter = RunPowerShellCommand(scriptMount, envVhdx).Trim();
            ...
            finally
            {
                ...
                var envVmFinally = new Dictionary<string, string> { { "VM_NAME", vmName } };

                if (!string.IsNullOrEmpty(mountPath))
                {
                    progress.Report("Desmontando VHDX de forma segura...");
                    string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", envVmFinally).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim() ?? string.Empty;
                    if (!string.IsNullOrEmpty(vhdxPath))
                    {
                         var envVhdx = new Dictionary<string, string> { { "VHDX_PATH", vhdxPath } };
                         RunPowerShellCommand("Dismount-VHD -Path $env:VHDX_PATH", envVhdx);
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand("Start-VM -Name $env:VM_NAME", envVmFinally);
        ...
        private (string Vendor, string DriverPath) DetectHostGpuAndDriver()
        {
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_VideoController"))
            {
                foreach (ManagementObject obj in searcher.Get())
                {
                    ...
                    var envGpu = new Dictionary<string, string> { { "GPU_NAME", name } };
                    string driverPath = RunPowerShellCommand(script, envGpu).Trim();
