        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    string vhdxPath2 = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(vhdxPath2))
                    {
                         envVars["VHDX_PATH_DISMOUNT"] = vhdxPath2;
                         RunPowerShellCommand("Dismount-VHD -Path $env:VHDX_PATH_DISMOUNT", envVars);
