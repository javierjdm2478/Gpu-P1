...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            var vmEnv = new Dictionary<string, string> { { "VM_NAME", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:VM_NAME | Select-Object -ExpandProperty State", vmEnv).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:VM_NAME -Force", vmEnv);
                ...
                RunPowerShellCommand("Set-VM -Name $env:VM_NAME -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", vmEnv);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:VM_NAME", vmEnv);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", vmEnv).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                ...
                var vhdxEnv = new Dictionary<string, string> { { "VHDX_PATH", vhdxPath } };
                ...
                string scriptMount = @"
                    ...
                    $vol.DriveLetter
                ";
                string driveLetter = RunPowerShellCommand(scriptMount, vhdxEnv).Trim();
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    string currentVhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", vmEnv).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(currentVhdxPath))
                    {
                         RunPowerShellCommand("Dismount-VHD -Path $env:VHDX_PATH", new Dictionary<string, string> { { "VHDX_PATH", currentVhdxPath } });
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand("Start-VM -Name $env:VM_NAME", vmEnv);
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_VideoController"))
            {
                foreach (ManagementObject obj in searcher.Get())
                {
                    ...
                    string script = @"
                        ...
                        $pattern = '*' + $env:GPU_NAME + '*'
                        $gpu = Get-CimInstance Win32_VideoController | Where-Object { $_.Name -like $pattern } | Select-Object -First 1
                        if ($gpu) {
                    ...
                    string driverPath = RunPowerShellCommand(script, new Dictionary<string, string> { { "GPU_NAME", name } }).Trim();
