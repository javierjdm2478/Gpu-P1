...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        {
            var envVars = new Dictionary<string, string> { { "TARGET_VM", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:TARGET_VM | Select-Object -ExpandProperty State", envVars).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:TARGET_VM -Force", envVars);
                ...
                RunPowerShellCommand("Set-VM -Name $env:TARGET_VM -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", envVars);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:TARGET_VM", envVars);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:TARGET_VM).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                ...
                envVars = new Dictionary<string, string> { { "TARGET_VHDX", vhdxPath } };
                string scriptMount = $@"
                    $vhd = Mount-VHD -Path $env:TARGET_VHDX -PassThru
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    envVars = new Dictionary<string, string> { { "TARGET_VM", vmName } };
                    string vhdxPathToDismount = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:TARGET_VM).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(vhdxPathToDismount))
                    {
                         envVars = new Dictionary<string, string> { { "TARGET_VHDX", vhdxPathToDismount } };
                         RunPowerShellCommand("Dismount-VHD -Path $env:TARGET_VHDX", envVars);
                ...
                if (wasVmRunning)
                {
                    ...
                    envVars = new Dictionary<string, string> { { "TARGET_VM", vmName } };
                    RunPowerShellCommand("Start-VM -Name $env:TARGET_VM", envVars);
