...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            // Sanitize input to prevent PowerShell command injection
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    string vhdxPathDismount = RunPowerShellCommand($"(Get-VMHardDiskDrive -VMName '{safeVmName}').Path").Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(vhdxPathDismount))
                    {
                         string safeVhdxPathDismount = vhdxPathDismount.Replace("'", "''");
                         RunPowerShellCommand($"Dismount-VHD -Path '{safeVhdxPathDismount}'");
