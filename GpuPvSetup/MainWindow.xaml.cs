...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        {
            // Prevenir inyección de comandos PowerShell pasando variables por entorno
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    string vhdxPathToDismount = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(vhdxPathToDismount))
                    {
                         var dismountEnvVars = new Dictionary<string, string> { { "VHDX_PATH", vhdxPathToDismount } };
