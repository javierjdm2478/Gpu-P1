        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    string cleanupVhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", vmEnvVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(cleanupVhdxPath))
                    {
                         var unmountEnvVars = new Dictionary<string, string> { { "VHDX_PATH", cleanupVhdxPath } };
                         RunPowerShellCommand("Dismount-VHD -Path $env:VHDX_PATH", unmountEnvVars);
