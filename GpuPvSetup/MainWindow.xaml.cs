...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        {
            var envVM = new System.Collections.Generic.Dictionary<string, string> { { "TARGET_VM", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:TARGET_VM | Select-Object -ExpandProperty State", envVM).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:TARGET_VM -Force", envVM);
                ...
                RunPowerShellCommand("Set-VM -Name $env:TARGET_VM -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", envVM);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:TARGET_VM", envVM);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:TARGET_VM).Path", envVM).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                ...
                string scriptMount = @"
                    ...
                    $vol.DriveLetter
                ";
                string driveLetter = RunPowerShellCommand(scriptMount, new Dictionary<string, string> { { "TARGET_VHDX", vhdxPath } }).Trim();
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    {
                         RunPowerShellCommand("Dismount-VHD -Path $env:TARGET_VHDX", new Dictionary<string, string> { { "TARGET_VHDX", vhdxPath } });
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand("Start-VM -Name $env:TARGET_VM", envVM);
