...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        {
            var env = new Dictionary<string, string> { { "VM_NAME", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand($"Get-VM -Name $env:VM_NAME | Select-Object -ExpandProperty State", env).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand($"Stop-VM -Name $env:VM_NAME -Force", env);
                ...
                RunPowerShellCommand($"Set-VM -Name $env:VM_NAME -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", env);
                RunPowerShellCommand($"Add-VMGpuPartitionAdapter -VMName $env:VM_NAME", env);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand($"(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", env).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    {
                         env["VHDX_PATH"] = vhdxPath;
                         RunPowerShellCommand($"Dismount-VHD -Path $env:VHDX_PATH", env);
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand($"Start-VM -Name $env:VM_NAME", env);
