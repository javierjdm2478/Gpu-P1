...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        {
            var envVars = new Dictionary<string, string> { { "TARGET_VM_NAME", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:TARGET_VM_NAME | Select-Object -ExpandProperty State", envVars).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:TARGET_VM_NAME -Force", envVars);
                ...
                RunPowerShellCommand("Set-VM -Name $env:TARGET_VM_NAME -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", envVars);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:TARGET_VM_NAME", envVars);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:TARGET_VM_NAME).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                ...
                var mountEnvVars = new Dictionary<string, string> { { "TARGET_VHDX_PATH", vhdxPath } };
                ...
                string scriptMount = @"
                    $vhd = Mount-VHD -Path $env:TARGET_VHDX_PATH -PassThru
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    {
                         var cleanEnvVars = new Dictionary<string, string> { { "TARGET_VHDX_PATH", vhdxPath } };
                         RunPowerShellCommand("Dismount-VHD -Path $env:TARGET_VHDX_PATH", cleanEnvVars);
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand("Start-VM -Name $env:TARGET_VM_NAME", envVars);
        ...
        private (string Vendor, string DriverPath) DetectHostGpuAndDriver()
        {
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_VideoController"))
            {
                foreach (ManagementObject obj in searcher.Get())
                {
                    ...
                    var gpuEnvVars = new Dictionary<string, string> { { "TARGET_GPU_NAME", name } };
                    string script = @"
                        $ErrorActionPreference = 'SilentlyContinue'
                        $gpu = Get-CimInstance Win32_VideoController | Where-Object { $_.Name -like '*'+$env:TARGET_GPU_NAME+'*' } | Select-Object -First 1
                        if ($gpu) {
