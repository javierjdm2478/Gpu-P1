        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    string currentVhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VM_NAME).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                    if (!string.IsNullOrEmpty(currentVhdxPath))
                    {
                         envVars["VHDX_PATH"] = currentVhdxPath;
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_VideoController"))
            {
                foreach (ManagementObject obj in searcher.Get())
                {
                    ...
                    string script = @"
                        ...
                        $targetName = $env:GPU_NAME
                        $gpu = Get-CimInstance Win32_VideoController | Where-Object { $_.Name -like ""*$targetName*"" } | Select-Object -First 1
                        if ($gpu) {
