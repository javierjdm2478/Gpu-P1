...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        private void ConfigureGpuPvAsync(string vmName, IProgress<string> progress)
        {
            ...
            var vmEnvVars = new Dictionary<string, string> { { "VmName", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:VmName | Select-Object -ExpandProperty State", vmEnvVars).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:VmName -Force", vmEnvVars);
                ...
                RunPowerShellCommand("Set-VM -Name $env:VmName -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", vmEnvVars);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:VmName", vmEnvVars);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VmName).Path", vmEnvVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                ...
                var mountEnvVars = new Dictionary<string, string> { { "VhdxPath", vhdxPath } };
                ...
                string scriptMount = @"
                    $vhd = Mount-VHD -Path $env:VhdxPath -PassThru
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    {
                         var unmountEnvVars = new Dictionary<string, string> { { "VhdxPath", vhdxPath } };
                         RunPowerShellCommand("Dismount-VHD -Path $env:VhdxPath", unmountEnvVars);
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand("Start-VM -Name $env:VmName", vmEnvVars);
        ...
        private (string Vendor, string DriverPath) DetectHostGpuAndDriver()
        {
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_VideoController"))
            {
                foreach (ManagementObject obj in searcher.Get())
                {
                    ...
                    var gpuEnvVars = new Dictionary<string, string> { { "GpuName", name } };
                    string script = @"
                        $ErrorActionPreference = 'SilentlyContinue'
                        $gpu = Get-CimInstance Win32_VideoController | Where-Object { $_.Name -like ""*$env:GpuName*"" } | Select-Object -First 1
                        if ($gpu) {
