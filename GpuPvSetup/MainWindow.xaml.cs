...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        {
            var envVars = new Dictionary<string, string> { { "VMNAME", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:VMNAME | Select-Object -ExpandProperty State", envVars).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:VMNAME -Force", envVars);
                ...
                RunPowerShellCommand("Set-VM -Name $env:VMNAME -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", envVars);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:VMNAME", envVars);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VMNAME).Path", envVars).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    {
                         envVars["VHDXPATH"] = vhdxPath;
                         RunPowerShellCommand("Dismount-VHD -Path $env:VHDXPATH", envVars);
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand("Start-VM -Name $env:VMNAME", envVars);
