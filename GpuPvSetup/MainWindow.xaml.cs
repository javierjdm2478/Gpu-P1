...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        {
            var env = new Dictionary<string, string> { { "VMNAME", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:VMNAME | Select-Object -ExpandProperty State", env).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:VMNAME -Force", env);
                ...
                RunPowerShellCommand("Set-VM -Name $env:VMNAME -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", env);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:VMNAME", env);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:VMNAME).Path", env).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    {
                         env["VHDXPATH"] = vhdxPath;
                         RunPowerShellCommand("Dismount-VHD -Path $env:VHDXPATH", env);
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand("Start-VM -Name $env:VMNAME", env);
