...
namespace GpuPvSetup
{
    public partial class MainWindow : Window
    {
        ...
        {
            var psEnv = new Dictionary<string, string> { { "TARGET_VM", vmName } };
            ...
            try
            {
                ...
                string vmState = RunPowerShellCommand("Get-VM -Name $env:TARGET_VM | Select-Object -ExpandProperty State", psEnv).Trim();
                ...
                if (vmState.Equals("Running", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    RunPowerShellCommand("Stop-VM -Name $env:TARGET_VM -Force", psEnv);
                ...
                RunPowerShellCommand("Set-VM -Name $env:TARGET_VM -GuestControlledCacheTypes $true -LowMemoryMappedIoSpace 3Gb -HighMemoryMappedIoSpace 33Gb", psEnv);
                RunPowerShellCommand("Add-VMGpuPartitionAdapter -VMName $env:TARGET_VM", psEnv);

                // 4. Montar VHDX
                progress.Report("Paso 4: Buscando y montando el disco VHDX...");
                string vhdxPath = RunPowerShellCommand("(Get-VMHardDiskDrive -VMName $env:TARGET_VM).Path", psEnv).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()?.Trim();
                ...
                string scriptMount = @"
                    ...
                    $vol.DriveLetter
                ";
                string driveLetter = RunPowerShellCommand(scriptMount, psEnv).Trim();
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(mountPath))
                {
                    ...
                    {
                         psEnv["TARGET_VHDX"] = vhdxPath;
                         RunPowerShellCommand("Dismount-VHD -Path $env:TARGET_VHDX", psEnv);
                ...
                if (wasVmRunning)
                {
                    ...
                    RunPowerShellCommand("Start-VM -Name $env:TARGET_VM", psEnv);
        ...
        private (string Vendor, string DriverPath) DetectHostGpuAndDriver()
        {
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_VideoController"))
            {
                foreach (ManagementObject obj in searcher.Get())
                {
                    ...
                    var psEnv = new Dictionary<string, string> { { "GPU_NAME", name } };
                    string script = @"
                    ...
                    string driverPath = RunPowerShellCommand(script, psEnv).Trim();
